	...
	if ($whatsapp_show_floating_box == 1) {
		...
		foreach ( $screens as $screen ) {
			add_meta_box(
				...
				esc_html__( 'Cresta WhatsApp Chat', 'cresta-whatsapp-chat' ),
...
function cresta_whatsapp_chat_metabox_callback( $post ) {
	...
	<label for="cresta_whatsapp_chat_new_field">
        <input type="checkbox" name="cresta_whatsapp_chat_new_field" id="cresta_whatsapp_chat_new_field" value="1" <?php checked( $crestaValue, '1' ); ?> /><?php esc_html_e( 'Hide Cresta WhatsApp Chat in this page?', 'cresta-whatsapp-chat' )?>
    </label>
	<?php
}
